...
import { createThermalSurfaceMaterial } from "@/lib/three/shaders/materials";
...
export function ThreeAnalysisHouse({ geometry, wallLayers, roofLayers, windows, vents, occupants, hvac, isRunning, simulationPoint = null, comfortBand = null, reduceMotion = false }) {
  ...
  useEffect(() => {
    ...
    const scene = new THREE.Scene(); scene.fog = new THREE.Fog(nightMode ? 0x0b1510 : 0xe6eadf, 10, 37); const disposeLandscape = addLandscapeContext(scene); const camera = new THREE.PerspectiveCamera(37, 1, .1, 100);
    ...
    const comfortLow = comfortBand?.t_low_c ?? 18;
    const comfortHigh = comfortBand?.t_high_c ?? 26;
    const thermalValue = simulationPoint ? THREE.MathUtils.clamp((simulationPoint.t_in_c - (comfortLow - 4)) / (comfortHigh - comfortLow + 8), 0, 1) : .5;
    ...
    if (simulationPoint) {
      const roofThermal = new THREE.Mesh(new THREE.PlaneGeometry(L * .88, W * .88), createThermalSurfaceMaterial({ value: thermalValue }));
      roofThermal.rotation.x = -Math.PI / 2;
      roofThermal.position.y = H + .045;
      house.add(roofThermal);
      const facadeThermal = new THREE.Mesh(new THREE.PlaneGeometry(L * .82, H * .68), createThermalSurfaceMaterial({ value: thermalValue, opacity: .34 }));
      facadeThermal.position.set(0, H * .53, W / 2 + .055);
      house.add(facadeThermal);
    }
    ...
    let frame;const render=(time)=>{frame=requestAnimationFrame(render);if(!reduceMotion&&!pointer.active)house.rotation.y=(orientationAngles[geometry?.orientation??"North"]??0)+Math.sin(time*.0002)*.04;if(!reduceMotion){scan.material.opacity=.06+(Math.sin(time*.002)+1)*.045;people.children.forEach((person,i)=>{person.position.y=Math.sin(time*.0015+i)*.015;});if(isRunning)scan.position.y=H+.03+Math.sin(time*.004)*.18;}renderer.render(scene,camera);};render(0);return()=>{cancelAnimationFrame(frame);observer.disconnect();renderer.domElement.removeEventListener("pointerdown",down);renderer.domElement.removeEventListener("pointermove",move);renderer.domElement.removeEventListener("pointerup",up);renderer.domElement.removeEventListener("pointerleave",up);renderer.domElement.removeEventListener("wheel",wheel);disposeLandscape();scene.traverse((item)=>{item.geometry?.dispose();if(Array.isArray(item.material))item.material.forEach((m)=>m.dispose());else item.material?.dispose();});renderer.dispose();renderer.domElement.remove();};
  },[geometry,wallLayers,roofLayers,windows,vents,occupants,hvac,isRunning,simulationPoint,comfortBand,nightMode,reduceMotion]);
